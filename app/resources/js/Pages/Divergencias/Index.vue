<template>
    <AppLayout>
        <div class="space-y-4">
            ...
            <!-- Lista -->
            <div class="space-y-3">
                <div v-if="!divergencias.data.length" class="bg-white rounded-xl shadow-sm p-10 text-center text-gray-400">
                    Nenhuma divergência encontrada.
                </div>

                <div v-for="d in divergencias.data" :key="d.id"
                    :class="['rounded-xl shadow-sm p-4 border', d.status === 'ABERTA' ? 'bg-red-50 border-red-200' : 'bg-white border-gray-200']">
                    <div class="flex flex-wrap items-start justify-between gap-3">
                        <div class="space-y-1">
                            <div class="flex items-center gap-2">
                                <span class="font-bold" :class="d.status === 'ABERTA' ? 'text-red-700' : 'text-gray-700'">
                                    {{ d.tipo_label }}
                                </span>
                                <StatusBadge :status="d.status" :label="d.status_label" />
                            </div>
                            <p class="text-sm text-gray-600">{{ d.descricao }}</p>
                            <p v-if="d.resolucao" class="text-sm text-green-700 mt-1">
                                ✓ {{ d.resolucao }}
                            </p>
                        </div>
                        <div class="text-sm text-right text-gray-500 space-y-0.5">
                            <div v-if="d.ordem">
                                <span class="font-bold text-gray-800">{{ d.ordem.placa }}</span>
                                — {{ d.ordem.motorista }}
                            </div>
        ...
        <div v-if="modalAberto" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
            <div class="bg-white rounded-xl shadow-xl w-full max-w-md p-6 space-y-4">
                ...
                <p class="text-sm text-gray-600">{{ divergenciaSelecionada?.tipo_label }}: {{ divergenciaSelecionada?.descricao }}</p>
